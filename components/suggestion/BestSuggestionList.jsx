...
export default function BestSuggestionList({ mySuggestion }) {
    ...
    const [showDetail, setShowDetail] = useState(()=>{
        const arrReject = mySuggestion.filter(filterBestSug)
        return arrReject
    })
    ...
    const dataBestSug = (allData) => {
        const arrReject = allData.filter(filterBestSug)
        ...
        const dataBestSug = [arrPrePrin, arrLam, arrFin, arrMain, arrQA, arrWH, arrReject]
        return dataBestSug
    }


    useEffect(() => {
        const arrImplemented = mySuggestion.filter(filterBestSug)
        setShowDetail(arrImplemented)
    },[confirmBtn])

    useEffect(() => {
        if(selectedValue === "Prepress-Printing"){
            setShowDetail(dataBestSug(mySuggestion)[0])
            }
        if(selectedValue === "Laminating"){
            setShowDetail(dataBestSug(mySuggestion)[1])
            }
        if(selectedValue === "Finishing"){
            setShowDetail(dataBestSug(mySuggestion)[2])
            }
        if(selectedValue === "Maintenance"){
            setShowDetail(dataBestSug(mySuggestion)[3])
            }
        if(selectedValue === "QA team"){
            setShowDetail(dataBestSug(mySuggestion)[4])
            }
        if(selectedValue === "Warehouse"){
            setShowDetail(dataBestSug(mySuggestion)[5])
            }
        if(selectedValue === "Select Department"){
            setShowDetail(dataBestSug(mySuggestion)[6])
            }
            ...
            {/* -----------Display form edit data-------- */}
                <div>
                    <div className='flex mb-5'>
                        <h2 className='text-amber-700 text-3xl font-bold mb-2 mt-5 mr-20'>Seclect Department To See Best Suggestion</h2>
                    </div>
                    <div className='flex mt-10 items-end'>
                        ...
                        <h2 className=' text-2xl font-bold ml-10'>No. Best Suggestion:</h2>
